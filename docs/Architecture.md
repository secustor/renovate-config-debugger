...
(`renovate/dist/config/**`) and records the pipeline trace: parse, migrate,
massage, validate, resolve presets, merge, and the packageRules simulator. Two
modules — and only two — hold that deep-import surface: `src/renovate-adapter.ts`
for engine code, and `src/shims/renovate-internals.ts` for the shims (which
...
holds the feature slices (editor, effective-config, overview, presets, session,
simulator); `src/app/` is the shell; `src/components/`, `src/hooks/`, `src/lib/`,
`src/data/` and `src/platform/` are shared. Features never import from `@/app`
and never from each other — oxlint overrides mechanize the rule. The engine is
imported dynamically, through one `loadEngine()` seam, so the critical path
stays small.
...
Node-only choke points for browser-safe shims in `src/shims/`: OpenTelemetry
instrumentation, the bunyan logger, `re2`, the datasource index, the package
cache, merge confidence, the hash util, conda's versioning module, and the seven
per-host preset clients (github, gitlab, gitea, forgejo, npm, http, local). The
logger shim doubles as the trace collector — **the preset tree and the
provenance events exist only in the shimmed module graph**, because the shims
are what emit them. Preset fetching becomes plain `fetch()` against CORS-enabled
host APIs.
...
projects over one set of fixtures and file snapshots: `golden` runs untouched
