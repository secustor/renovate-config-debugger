# 058 — `rcd`: the debugger CLI on the shimmed engine
...
module graph. This item adds `packages/cli`: an `rcd` bin that hosts the
...
- `packages/cli` workspace package, bin `rcd`, built on the spike's runner
...
- Tokens from env only (`RCD_*_TOKEN`, falling back to `GITHUB_TOKEN`/
...
- CLAUDE.md wires `rcd` in as the sanctioned way to debug resolution;
...
- **Exit `2` = config refused, deliberately.** Claude Code hooks treat exit
  ...
  `rcd validate` drops straight into a Stop/PreToolUse hook with no wrapper.
...
- **Relation to `renovate-config-validator`:** upstream is the linter —
  pass/fail on the file as written, no preset resolution. `rcd` is the
  debugger; both run the same pinned `renovate` code, so they cannot
  disagree about semantics.

## As built (2026-08-05)

- `packages/cli`: `bin/rcd.mjs` boots `createServer({ configFile: vite.config.ts })`
...
## Follow-up: publishing `rcd` as a second product
