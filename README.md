...
- Private presets and private repo configs need auth. The GitHub App's only
  ...
  and fires a best-effort `POST /logout`, since only the worker can drop the
  refresh cookie; the chip links to GitHub's authorization page for true
  revocation.
...
the platform + endpoint picked under _Advanced — hosts & credentials_ →
_Repository host_ (default `github` / `https://api.github.com`), and the trace
records which one each node used.
