...
import { isTextEditingTarget, mayOwnNativePopup } from "./keyboard-target";

/**
 * Roadmap 068 — `isTextEditingTarget` is shared by the 016 Home/End page-scroll
 * guard (`hooks/scroll-ergonomics.ts`) and the bare-key jump layer
 * (`useShortcut`, `useTabDigits`), and `mayOwnNativePopup` is what the Escape
 * ladder yields to. Both need real DOM elements (`instanceof HTMLElement`,
 * `.tagName`, `.closest`), which the node-environment `unit` project doesn't
 * have — hence `.test.tsx` here, to land in the jsdom `render` project
 * (`lib/focusable.test.tsx` is the precedent).
 *
